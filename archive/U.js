...
var server = http.createServer(function(req, res) {
  res.writeHead(200);
  let ressource = decodeURIComponent(req.url === "/favicon.ico" ? "" : req.url.substring(1));
    console.log(p(ressource));
  let method = req.method;
    res.end(`U has been requested with ${method} on ${ressource}`);
}).listen(8080);
function loadMemoryFrom(file,mode) {
    if (mode == "ONLINE") {
        new AWS.S3({
            accessKeyId: '',
            secretAccessKey: ''
        }).getObject({ Bucket:'us-memories', Key: 'U.json' }, function(err, data) {
            if (!err) {
                memory = JSON.parse(data.Body.toString());
                console.log("memory loaded");
            }
            else 
                console.log("enable to load memory");
        });
    }
    else {
...
function saveMemoryTo(file,mode,callback) {
  if (mode == "ONLINE") {
    ...
    var s = new Readable();
    s.push(JSON.stringify(memory, null, '\t'));  // the string you want
    s.push(null);
    s.pipe(upload);
  }
  else {
    fs.writeFile("U.json",function(err) {
        if (!err) {
            callback();
        }
    });
...
function addToMemory(plural,singular,relation) {
  var in_m=memory[plural];
  memory[plural]=in_m?(in_m+"&("+singular+")"):"("+singular+")";
  var in_m2=memory[singular];
  if (relation=="position")
    memory[singular]=in_m2?(in_m2+".("+plural+")"):"("+plural+")";
  else if (relation=="type")
    memory[singular]=in_m2?(in_m2+".("+plural.substring(0,plural.length-1)+")"):"("+plural.substring(0,plural.length-1)+")";
}
function readFromMemory(key) {
  ...
  if (!data)
    ...
    var parsed = p(data);
    if ("&_?.".indexOf(parsed[0].charAt(0))==-1)
        return [decapsulated(data)];
    else 
        return parsed.slice(1);
  }
}
function anU(c) {
  return (c!="" && (c=="(" || c==")" || c=="+" || c=="-" || c=="#" || c=="/" || c=="." || c=="@" || c==":" || c=="_" || c=="&" || c=="?" || c=="%"))
}
function aD(c) {
  return (c!="" && (c=="0" || c=="1" || c=="2" || c=="3" || c=="4" || c=="5" || c=="6" || c=="7" || c=="8" || c=="9"))
}
function aM(c) {
  return (c!="" && (c=="A" || c=="B" || c=="C" || c=="D" || c=="E" || c=="F" || c=="G" || c=="H" || c=="I" || c=="J" || c=="K" || c=="L" || c=="M" || c=="N" || c=="O" || c=="P" || c=="Q" || c=="R" || c=="S" || c=="T" || c=="U" || c=="V" || c=="W" || c=="X" || c=="Y" || c=="Z"))
}
function decapsulated(text) {
  if (text.charAt(text.length-1)==")")
    return text.substring(text.charAt(0)=="("?1:0,text.slice(-1)==")"?text.length-1:text.length);
  return text;
}
function pointified(text) {
  return anU(text.charAt(text.length-1))?text:(text+".");
}
function p(t) {
  var parsed=[];
  if (!t)
...
loadMemoryFrom("U.json",MEMORY_MODE);
